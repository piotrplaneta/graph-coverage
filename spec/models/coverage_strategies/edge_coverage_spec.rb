...
describe Graph do
  ...
  before { subject.extend(EdgeCoverage) }
  ...
  describe "coverage tests" do
    ...
    let(:covering_test) do
      ...
      path2 = Path.new([nodes[0], nodes[2], nodes[3], nodes[1], nodes[4]])
    ...
    describe "#covered_with?" do
      it "returns true if graph is edge covered by test" do
        expect(subject).to be_covered_with(covering_test)
      end

      it "returns false if graph is edge covered by test but test is invalid" do
        expect(subject).to_not be_covered_with(covering_invalid_test)
      end

      it "returns false if graph is not edge coveraged by test" do
